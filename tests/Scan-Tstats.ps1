  <#
.SYNOPSIS
  SSDP discover for RadioThermostat/CT50 devices (uses ST=com.rtcoa.tstat*), then verifies via /sys and /sys/name.

.DESCRIPTION
  - Sends an SSDP M-SEARCH (ST: com.rtcoa.tstat*) to multicast 239.255.255.250:1900
  - Collects LOCATION headers from replies
  - For each unique LOCATION, queries host's /sys and /sys/name (only)
  - Validates /sys (requires fw/api/uuid or a non-empty /sys/name that's not an HTML status)
  - Prints Name, IP, Version, LOCATION
#>
...
# Build M-SEARCH payload (normalize CRLF)
$msearch = @"
M-SEARCH * HTTP/1.1
...
try {
  ...
  while ($sw.Elapsed.TotalSeconds -lt $TimeoutSeconds) {
    try {
      ...
      # receive timeout -> continue checking total time
      continue
    } catch {
      continue
    }
...
function Is-Thermostat {
  param($fetch)
  # Accept when /sys parsed object contains known keys
  if ($fetch.Sys -is [System.Management.Automation.PSCustomObject]) {
    ...
    # accept if there are non-empty properties (not nested empty arrays)
  ...
  # If SysRaw looks like HTML status page -> reject unless /sys/name gives a valid non-HTML string
  ...
  # fallback: if /sys/name is a plain non-empty string, accept
...
# helper to extract name/version
function Extract-Info {
  ...
  # version: prefer fw_version then api_version
  if ($fetch.Sys -is [System.Management.Automation.PSCustomObject]) {
...
foreach ($loc in $locations) {
  ...
  # build base: use scheme and host:port (so we can call /sys reliably)
  $base = ("{0}://{1}:{2}" -f $u.Scheme, $u.Host, $u.Port)
  # if port is default for scheme, keep it (no harm)
...
# dedupe by IP (keep first) and print
